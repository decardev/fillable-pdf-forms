% !TeX root = ./PatientData_imageInpu.tex
...
\startJScode {textStyleInit} 	event.target.textFont = font.Helv; event.target.textSize=8 \stopJScode
\startJScode {textOnlyNumbers}  if(!event.willCommit){event.rc = /^[0-9._\b]*$/.test(event.change)};//$ \stopJScode
...
\setupfield[TextLineLong][height=4.5mm, width=70mm, frame=on, offset=0mm] % regionin=JS(textStyleInit), validate=JS(textToUpperCase)]
%\setupfield[TextLineMedium][][[height=4.5mm, width=40mm, frame=on, offset=0mm]]
\setupfield[TextLineShort][height=4.5mm, width=9mm, frame=on, offset=0mm, regionin=JS(textStyleInit), afterkey=JS(textOnlyNumbers)]
...
\definefield[mybox][push][ImageBox]


\definefield[checkme][check][CheckBox][yes,no][no]
%\definefield[Ugly2][popup][UglySetup][UGLY,AWFUL,BAD] [AWFUL]
\definebutton[testbutton][MyButton] %\button[frame=on, width=48.2mm, height=39.4mm]{}[NextPage]
